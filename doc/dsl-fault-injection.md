...
## What each kind will do, once wired
...
Each hook is one `Active()` lookup guarded by a null check, placed beside the
existing behaviour rather than inside it, so the normal path is not
restructured. Relay of *other* nodes' messages is deliberately not a fault
