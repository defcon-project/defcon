# CJDNS support in DeFCoN Core

It is possible to run DeFCoN Core over CJDNS, an encrypted IPv6 network that
...
done outside of DeFCoN Core, similarly to a VPN (either in the host/OS or on
the network router).
...
## Run DeFCoN Core with CJDNS

Once you are connected to the CJDNS network, the following DeFCoN Core
...
When enabled, this option tells DeFCoN Core that it is running in an
...
IPv6 local network. This helps DeFCoN Core perform better address management:
...
## CJDNS-related information in DeFCoN Core

There are several ways to see your CJDNS address in DeFCoN Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `defcon-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `defcon-cli getpeerinfo`).
