# User-space, Statically Defined Tracing (USDT) for DeFCoN Core

DeFCoN Core includes statically defined tracepoints to allow for more
...
**Note**: DeFCoN Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to DeFCoN Core
...
Multiple tools can list the available tracepoints in a `defcond` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in DeFCoN Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/defcond
stap net        outbound_message 0x0000000000107c05 /src/defcond
stap validation block_connected  0x00000000002fb10c /src/defcond
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in DeFCoN Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/defcond | grep NT_STAPSDT -A 4 -B 2
