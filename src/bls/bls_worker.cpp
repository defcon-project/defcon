                                               ...
                                               bool parallel, bool aggregated, std::function<void(const std::vector<bool>&)> doneCallback)
{
    ...
    // (broken_promise). CDKGSession::VerifyPendingContributions can reach this
    // path when every pending member has been marked bad between enqueue and
    // flush (e.g. double-contribution MarkBadMember). Mirror the empty-input
