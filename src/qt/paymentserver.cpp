...
const QString BITCOIN_IPC_PREFIX("defcon:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // defcon: URI
        {
    ...
    optionsModel(nullptr)
{
    ...
    // on Mac: sent when you click defcon: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start defcon: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling defcon: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("defcon://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'defcon://' is not a valid URI. Use 'defcon:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // defcon: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid DeFCoN address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
