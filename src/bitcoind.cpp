...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/pacplatform.conf are parsed in qt/bitcoin.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (!args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo()) + "\n"
                "\nUsage:  pacplatformd [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see pacplatformd -h for a list of options.\n", argv[i])));
            }
        }
        ...
        // -server defaults to true for pacplatformd but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect pacplatformd signal handlers
    noui_connect();
