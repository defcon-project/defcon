    ...
    return RPCHelpMan{
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::STR_HEX, "proTxHash", /*optional=*/true, "The proTxHash the report below is about"},
                {RPCResult::Type::ARR, "upcoming_dkgs", /*optional=*/true, "Upcoming DKG sessions for the given proTxHash whose work block is already mined",
                {
                    {RPCResult::Type::OBJ, "", "",
                    {
                        {RPCResult::Type::NUM, "llmqType", "The LLMQ type"},
                        {RPCResult::Type::NUM, "quorumIndex", "The quorum index"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const auto quorum_type_known_enabled = [&](Consensus::LLMQType llmq_type, int quorum_base_height) {
        ...
        if (quorum_base_predecessor_height <= nTipHeight) {
            ...
            return pQuorumBasePredecessor != nullptr && llmq::IsQuorumTypeEnabled(llmq_type, pQuorumBasePredecessor);
        }
        ...
        // in for the rest.
        return llmq::IsQuorumTypeEnabledInternal(llmq_type, pindexTip,
    ...
    } else if (node.mn_activeman) {
        proTxHash = node.mn_activeman->GetProTxHash();
    }
    ...
    if (!proTxHash.IsNull()) {
        ...
        for (const auto& llmq_params : consensus.llmqs) {
            ...
            for (const int quorumIndex : irange::range(quorums_num)) {
                ...
                // A rotation-capable type without DIP0024 active at its cycle base has no
                // canonical member selection; this can only happen around activation.
                if (llmq_params.useRotation &&
                ...
                // All indices of a rotated cycle share the work block of their cycle base;
                // for non-rotated types cycleBaseHeight == quorumHeight anyway.
                const int workHeight{cycleBaseHeight - 8};
                if (workHeight > nTipHeight) {
                    continue;
                }

                const CBlockIndex* const pWorkBlockIndex = WITH_LOCK(cs_main, return pindexTip->GetAncestor(workHeight));
                ...
                const auto predicted_members = llmq::utils::ComputeQuorumMembersFromWorkBlock(
                    llmq_params.type, *CHECK_NONFATAL(node.dmnman), pWorkBlockIndex, quorumHeight);
                if (!predicted_members.has_value()) {
                    obj.pushKV("known", false);
                    obj.pushKV("reason", "rotated quorum member prediction is not available");
                    upcoming.push_back(obj);
                    continue;
                }
