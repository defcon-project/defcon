    ...
    while (!interruptNet)
    {
        ...
        if (add_fixed_seeds && !fixed_seed_networks.empty()) {
            ...
            if (add_fixed_seeds_now) {
                ...
                // Queue direct address-fetch connections as well, which upstream
                // does not: a seed that answers supplies peers at once instead of
                // waiting for the next outbound slot. This is about what happens
                // AFTER the decision above, and is deliberately kept -- what was
                // wrong was making that decision immediately, not fetching from
                // the seeds once it is made.
                for (const CAddress& seed_addr : seed_addrs) {
                    AddAddrFetch(seed_addr.ToStringAddrPort());
                }
                add_fixed_seeds = false;
                LogPrintf("Added and queued %d fixed seeds from reachable networks.\n", seed_addrs.size());
            }
        }
