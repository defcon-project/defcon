 ...
 */
static const char* BITCOIN_PID_FILENAME = "defcond.pid";
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-staking", strprintf("Run the staking subsystem, which starts the staking thread (default: %u). This is not the setstaking RPC: that enables one wallet for staking, is off for every wallet at startup, and is not stored, so it has to be set again after a restart", DEFAULT_STAKING), ArgsManager::ALLOW_BOOL, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-sporkaddr=<defconaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
    ...
    argsman.AddArg("-masternodeblsprivkey=<hex>", strprintf("Set the masternode BLS private key and enable the client to act as a masternode. A masternode needs -txindex=1, -peerbloomfilters=1, -listen=1, no -prune and at least -maxconnections=%u, and it runs with the wallet disabled -- the node refuses to start otherwise.", DEFAULT_MAX_PEER_CONNECTIONS), ArgsManager::ALLOW_ANY | ArgsManager::SENSITIVE, OptionsCategory::MASTERNODE);
    // Inherited from a chain that runs a second layer on top of its masternodes.
    // This one does not, so the restricted RPC user the option names has
    // nothing to serve; the machinery stays for compatibility and stays out
    // of the help.
    argsman.AddArg("-deprecated-platform-user=<user>", "Set the username of the restricted RPC user on a masternode (inherited; no second-layer service uses it on this chain).", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::MASTERNODE);
...
std::string LicenseInfo()
{
    return std::string{} +
           ...
           "\n" +
           "Please contribute if you find DeFCoN useful. Visit <https://www.dfcn.io/> for more information about the software.\n" +
           "The source code is available at <https://github.com/defcon-project/defcon>.\n" +
           "\n" +
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    node.peerman = PeerManager::make(chainparams, *node.connman, *node.addrman, node.banman.get(),
                                     ...
                                     *node.govman, *node.sporkman, node.mn_activeman.get(), node.dmnman,
                                     node.cj_ctx, node.llmq_ctx, node.dslman, ignores_incoming_txs,
                                     node.faultinjector.get());
