...
// Regression for the guard our #67 brought in (dash#7486): an empty
// verification-vector set schedules no work at all, so before the guard nothing
// ever invoked doneCallback and .get() threw std::future_error(broken_promise).
// In production that throw escapes VerifyPendingContributions on the DKG
// phase-handler thread, which only catches AbortPhaseException, and
// std::terminate aborts defcond.
...
void FuncVerifyContributionSharesHonest(const bool legacy_scheme)
{
    ...
    for (const size_t i : irange::range(QUORUM_SIZE)) {
        uint256 id;
    ...
    for ([[maybe_unused]] const size_t i : irange::range(QUORUM_SIZE)) {
