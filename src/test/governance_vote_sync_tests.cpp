                               ...
                               vote_signal_enum_t signal, int64_t time)
{
    ...
    vote.SetTime(time);
    vote.SetSignature(sk.Sign(vote.GetSignatureHash(), /*specificLegacyScheme=*/false)
...
// resets it by reconnecting. But this map is shared with the per-peer outbound
// sync bookkeeping ("full-sync", "governance-sync", "spork-sync") and the govsync
// quota is coupled to a misbehaviour score of 20. Several honest nodes
// legitimately share one address (NAT, colocation, CI), and collapsing them onto
// one key makes the second honest peer look like a repeat offender -- the ban
// score escalates to discouragement, and mnsync breaks outright on regtest,
// where every node is on 127.0.0.1.
//
...
BOOST_AUTO_TEST_CASE(fulfilled_request_manager_is_port_sensitive)
{
    ...
    const std::string request{"govsync-votes-deadbeef"};
...
BOOST_AUTO_TEST_CASE(vote_signature_verification_is_cached)
{
    ...
    CGovernanceVote vote{MakeSignedVote(sk, COutPoint{uint256S("01"), /*nIn=*/0}, uint256S("02"), VOTE_SIGNAL_VALID,
                                        1'700'000'000)};
...
BOOST_AUTO_TEST_CASE(stored_vote_file_reuses_signature_cache)
{
    ...
    CGovernanceVote vote{MakeSignedVote(sk, COutPoint{uint256S("11"), /*nIn=*/1}, uint256S("22"), VOTE_SIGNAL_DELETE,
                                        1'700'000'100)};
...
// the signed payload (GetSignatureString/GetSignatureHash) but is mutable via
// SetTime(), which the vote-relay paths call on reconstructed votes before
// verifying them. A key-only memo would return the stale "valid" verdict for
// content that was never signed -- a signature-check bypass.
BOOST_AUTO_TEST_CASE(signature_cache_invalidated_by_signed_field_mutation)
{
    ...
    CGovernanceVote vote{MakeSignedVote(sk, COutPoint{uint256S("33"), /*nIn=*/2}, uint256S("44"), VOTE_SIGNAL_FUNDING,
                                        1'700'000'200)};
...
BOOST_AUTO_TEST_CASE(signature_cache_does_not_confuse_key_types)
{
    ...
    CGovernanceVote vote{MakeSignedVote(sk, COutPoint{uint256S("55"), /*nIn=*/3}, uint256S("66"), VOTE_SIGNAL_VALID,
                                        1'700'000'300)};
