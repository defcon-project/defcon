                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (fProofOfStake && (pindex->nHeight <= m_params.GetConsensus().lastPowBlock)) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "pos-early");
    } else if (!fProofOfStake && (pindex->nHeight > m_params.GetConsensus().lastPowBlock)) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "pow-late");
    }
    ...
    if (!m_chain_helper->special_tx->ProcessSpecialTxsInBlock(block, pindex, view, fJustCheck, fScriptChecks, state, mnlist_updates_opt)) {
        return error("ConnectBlock(DFCN): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // DFCN : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (m_chain_helper->ShouldInstantSendRejectConflicts()) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            while (auto conflictLockOpt = m_chain_helper->ConflictingISLockIfAny(*tx)) {
                ...
                if (has_chainlock) {
                    LogPrint(BCLog::ALL, "ConnectBlock(DFCN): chain-locked transaction %s overrides islock %s\n", tx->GetHash().ToString(), conflict_islock_hash.ToString());
                    m_chain_helper->RemoveConflictingISLockByTx(*tx);
                } else {
                    // The node which relayed this should switch to correct chain.
                    // TODO: relay instantsend data/proof.
                    LogPrintf("ERROR: ConnectBlock(DFCN): transaction %s conflicts with transaction lock %s\n", tx->GetHash().ToString(), conflict_txid.ToString());
    ...
    // DFCN : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!m_chain_helper->special_tx->CheckCreditPoolDiffForBlock(block, pindex, blockSubsidy, state)) {
        return error("ConnectBlock(DFCN): CheckCreditPoolDiffForBlock for block %s failed with %s",
                     pindex->GetBlockHash().ToString(), state.ToString());
    }
    ...
    // Block is committed: keep the scheme it switched to. Dry runs returned above.
...
bool CChainState::DisconnectTip(BlockValidationState& state, DisconnectedBlockTransactions* disconnectpool)
{
    ...
    // DisconnectBlock may switch the BLS scheme back while the active tip still
    // points at the disconnected block. Restore it if post-processing fails.
    ScopedBLSLegacyScheme bls_scheme_guard;
...
bool CChainState::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    ...
    // ConnectBlock commits its scheme switch before the active tip advances.
    // Restore it if a later operation fails before SetTip().
    ScopedBLSLegacyScheme bls_scheme_guard;
    // Read block from disk.
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature)
{
    ...
    if (!CheckBlockHeader(block, block.GetHash(), state, consensusParams, fCheckPOW && !block.IsProofOfStake()))
        return false;
    ...
    // Ensure coinstake is unique with correct placement
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (block.IsProofOfStake() && !CheckCoinStakeTimestamp(block.GetBlockTime(), consensusParams)) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-coinstake-time", "coinstake timestamp violation");
    }
    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    // ConnectBlock below can switch the scheme without advancing the active tip.
    // VerifyDB is observational, so restore the process-wide scheme on return.
    ScopedBLSLegacyScheme bls_scheme_guard;
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs)
{
    ...
    if (!m_chain_helper->special_tx->ProcessSpecialTxsInBlock(block, pindex, inputs, false /*fJustCheck*/, false /*fScriptChecks*/, state, mnlist_updates_opt)) {
        return error("RollforwardBlock(DFCN): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), state.ToString());
    }
    ...
    if (fAddressIndex) {
        if (!m_blockman.m_block_tree_db->WriteAddressIndex(addressIndex)) {
            return error("RollforwardBlock(DFCN): Failed to write address index");
        }

        if (!m_blockman.m_block_tree_db->UpdateAddressUnspentIndex(addressUnspentIndex)) {
            return error("RollforwardBlock(DFCN): Failed to write address unspent index");
        }
    }

    if (fSpentIndex) {
        if (!m_blockman.m_block_tree_db->UpdateSpentIndex(spentIndex))
            return error("RollforwardBlock(DFCN): Failed to write transaction index");
    }

    if (fTimestampIndex) {
        if (!m_blockman.m_block_tree_db->WriteTimestampIndex(CTimestampIndexKey(pindex->nTime, pindex->GetBlockHash())))
            return error("RollforwardBlock(DFCN): Failed to write timestamp index");
    }
...
bool CChainState::ReplayBlocks()
{
    ...
    if (!hashHeads[1].IsNull()) { // The old tip is allowed to be 0, indicating it's the first flush.
        ...
        if (fDIP0003Active && !m_evoDb.VerifyBestBlock(pindexOld->GetBlockHash())) {
            return error("ReplayBlocks(DFCN): Found EvoDB inconsistency");
        }
    }
