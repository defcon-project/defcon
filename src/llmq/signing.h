...
public:
    ...
    // This won't completely remove all traces of the recovered sig but instead leave the hash and signHash entries in
    // the DB. This allows AlreadyHave/late-share filtering to keep returning true. Cleanup will later remove the remains
    void TruncateRecoveredSig(Consensus::LLMQType llmqType, const uint256& id);
