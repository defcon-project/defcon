...
struct Peer {
     ...
     *  block and granted up front only to a connection this node opened or a
     *  verified masternode, both in
 ...
 *  than unsolicited (see CNodeState::m_recent_object_requests). GetObjectInterval() is already this
 *  node's statement of how long it is willing to wait before asking someone else, so the grace is
 *  kept in the same currency. In this tree an in-flight request itself survives until the expiry
 *  sweep (GetObjectExpiryInterval), which is far longer -- the grace exists for requests whose
 *  bookkeeping was erased early because the object was accepted from another source. (dash#7484,
 *  adapted) */
 ...
 *  CNodeState::m_recent_object_requests). Evicting early costs the late-answer grace for the oldest
...
struct CNodeState {
    ...
    //! The GETDATA-only objects (see IsGetDataOnlyObject) we have recently asked this peer for.
...
public:
    ...
    GetDataResponse ConsumeGetDataResponse(NodeId nodeid, const CInv& inv) override EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
...
private:
     ...
     *  operator key, do not exist here yet -- so what the hold knows about an
     *  entry is who vouches for it: every LIVE peer that delivered a copy.
     *  The flood carries a genuine announcement through every honest peer, so
     *  a genuine entry gathers vouchers as the base approaches; junk is
     *  vouched for only by whoever made it up, and a peer that disconnects
     *  takes its vouching with it (ForgetDSLEarlyVoucher) -- credit does not
...
GetDataResponse PeerManagerImpl::ConsumeGetDataResponse(NodeId nodeid, const CInv& inv)
{
    AssertLockHeld(cs_main);

    CNodeState* state = State(nodeid);
    if (state == nullptr) return GetDataResponse::UNREQUESTED;

    if (state->m_object_download.m_object_in_flight.count(inv) > 0) {
        // Answered while the request was still in flight. Consume the request, and spend the
        // late-answer grace too, so the one GETDATA cannot also pay for a replay of the payload.
        EraseObjectRequest(nodeid, inv);
        state->m_recent_object_requests.erase(inv.hash);
        return GetDataResponse::REQUESTED;
    }

    // No in-flight request. Before treating this as an unsolicited push, check whether we asked
    // this peer for it at all: the in-flight entry is erased once the object is accepted from any
    // source, and the expiry sweep drops requests answered too slowly. Neither means the peer
    // misbehaved.
    ...
    if (state->m_recent_object_requests.get(inv.hash, requested)) {
        ...
        if (requested.m_inv_type == inv.type &&
            ...
            // Tidy whatever bookkeeping is left, exactly as an on-time answer would have.
            EraseObjectRequest(nodeid, inv);
...
bool IsGetDataOnlyObject(int invType)
{
    ...
    // are the GETDATA handler (ProcessGetData), and no local producer pushes them. Receiving one we
    // never asked for is therefore always unsolicited, which lets the handlers drop it before doing
    // any work on the sender's behalf.
    //
    // Not every inv-driven type belongs here. MSG_QUORUM_RECOVERED_SIG (proactive relay to peers
    // that sent QSENDRECSIGS), MSG_DSQ (SENDDSQUEUE), MSG_ISDLOCK (pushed alongside MERKLEBLOCK for
    // BIP37 clients) and MSG_SPORK (bulk push in reply to GETSPORKS) all have a legitimate
    // unsolicited-push path. (dash#7484, adapted)
    switch (invType) {
        case MSG_CLSIG:
    ...
    const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::FILTERLOAD) {
        ...
        try {
            ...
            // An oversized filter now throws pre-allocation; punish here instead of the outer catch. (dash#7444)
    ...
    if (found)
    {
        ...
        PostProcessMessage(m_llmq_ctx->quorum_block_processor->ProcessMessage(
                               ...
                               [this, &pfrom](const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(!::cs_main) {
                                   return WITH_LOCK(::cs_main,
                                                    return ConsumeGetDataResponse(pfrom.GetId(), inv)) !=
        ...
        if (msg_type == NetMsgType::CLSIG) {
            if (llmq::AreChainLocksEnabled(m_sporkman)) {
                ...
                // disabled does not burn a later retransmit. (dash#7484, adapted)
                if (WITH_LOCK(::cs_main, return ConsumeGetDataResponse(pfrom.GetId(), CInv{MSG_CLSIG, hash})) ==
                    GetDataResponse::UNREQUESTED) {
                    LogPrint(BCLog::CHAINLOCKS, "CLSIG -- received unrequested CLSIG %s, peer=%d\n",
                             hash.ToString(), pfrom.GetId());
                    Misbehaving(pfrom.GetId(), UNREQUESTED_OBJECT_MISBEHAVIOR_SCORE, "unrequested clsig");
                    return;
                }
 ...
 *  two where a reorg had a masternode sign against a second base.
 *
 *  What the size decides is how many connections it takes to crowd honest
 *  entries out. On a node that fell behind, every honest peer delivers the
 *  same N announcements, so each holds a share of N in the contest of
 *  HoldDSLEarlyResponse, and k attacking connections splitting the rest of a
 *  hold of S entries start to win once k > S/N - 1. At four entries per
 *  masternode that was four connections on any network size; at 28 it takes
 *  28, about the number of peers each honest message arrived through on the
 *  devnet (29). A reorg epoch doubles the honest share and halves that.
 *
 *  What the size no longer decides is the drain. Every held entry naming a real
 *  masternode used to cost a signature check once the base connected, which is
 *  why the previous round tied the hold to four per masternode and made
 *  crowding cheap; the drain now stops checking entries vouched for only by
 *  peers that already delivered a bad signature (ProcessDSLTick), so what it
 *  pays follows the number of connections, not the size of the hold. */
static constexpr size_t DSL_EARLY_RESPONSES_PER_MN{28};
/** The hold never shrinks below this, whatever the list: a small network gets
 *  the bound a fixed hold always had. */
static constexpr size_t DSL_EARLY_RESPONSES_MIN{4096};
/** Nor grows past this. Once the hold is full, every further entry is weighed
 *  against all of it, which is linear in its size -- about a millisecond on the
 *  message thread here. Above 585 masternodes this caps it, and the number of
 *  connections that crowds honest entries out falls below 28 (16 at 1000). */
static constexpr size_t DSL_EARLY_RESPONSES_MAX{16384};
...
{
    const size_t sized{static_cast<size_t>(std::max(mn_count, DSL_MSG_BUDGET_MIN_MNS)) * DSL_EARLY_RESPONSES_PER_MN};
    return std::clamp(sized, DSL_EARLY_RESPONSES_MIN, DSL_EARLY_RESPONSES_MAX);
}
...
bool PeerManagerImpl::ChargeDSLMessageBudget(const CNode& pfrom, Peer& peer)
{
    ...
    // A connection starts with nothing it has not earned, unless this node
    // opened it or it is a verified masternode. Credit handed to a bare inbound
    // connection up front is
    ...
    // all quorum links (#208) must not drop the burst that follows. That credit
    // belongs to the identity. MNAUTH needs the operator key of a registered
    // masternode (PoSe-banned ones included; the collateral stays locked), and
                                           ...
                                           size_t hold_max)
{
    ...
    auto& epoch = m_dsl_early_responses[resp.nEpoch];
    ...
    const auto vouch = [&](DSLEarlyResponse& entry) {
    ...
    // once per peer, so a genuine one arrives from several, and every one of
    // them joins its vouchers -- which is also why a repeat costs a flooder
    // nothing and buys it nothing.
    const auto same = std::find_if(held.begin(), held.end(), [&resp](const DSLEarlyResponse& entry) {
    ...
    // what the drain pays is bounded by the peers that vouch (ProcessDSLTick),
...
void PeerManagerImpl::ProcessDSLTick(const CBlockIndex* pindexNew)
{
    ...
    // Announcements held back because they arrived before this epoch's base
    // block did (ProcessDSLMessage): the base is connected now, so they are
    // processed and, if accepted, relayed -- what the flood would have done
    ...
    // every node drains, the miner included, since the pool lives there too.
    {
        ...
        if (!early.empty()) {
            ...
            LogPrint(BCLog::NET, "DSL -- %d of %d held announcement(s) for epoch %d accepted once its base block connected "
                     ...
                     accepted_count, early.size(), epoch, refused_count, skipped_count);
        }
    }
            ...
            if (!AlreadyHave(inv)) {
                ...
                if (last_request_time <= current_time - GetObjectInterval(inv.type)) {
                    ...
                    if (IsGetDataOnlyObject(inv.type)) {
                        // Remember that we asked, so that an answer arriving after the in-flight
                        // entry is gone -- erased because the object turned up elsewhere, or swept
                        // by expiry -- is not mistaken for an unsolicited push. See GetDataResponse.
                        state.m_recent_object_requests.insert(inv.hash,
                                                              RequestedObject{static_cast<uint32_t>(inv.type), current_time});
                    }
                } else {
