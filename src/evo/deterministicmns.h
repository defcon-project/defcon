...
private:
    ...
    {
        // A BLS operator key can keep the same point while its serialized encoding
        // (legacy<->basic) changes on a version transition. The map is keyed by
        // GetUniquePropertyHash(), so only that hash reveals the entry must be re-keyed to the
        // new scheme; CBLSLazyPublicKey::operator== compares the point and ignores the scheme,
        // so it would wrongly short-circuit. Compare the serialized hashes for BLS keys and the
        // plain value for every other unique property.
...
public:
    ...
    // Variant taking an explicit starting list instead of loading it through
    // GetListForBlock. Used to rebuild diffs from a trusted snapshot, where the
    // list to build on is the snapshot rather than whatever evodb currently
    // holds -- which is the thing under repair.
